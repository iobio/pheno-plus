<template>
  <div id="term-peek-div" :class="{ visible: hpoItemObj }">
    <div class="full-note-overlay" v-if="fullNoteShown && noteSelected">
      ...
      <div
        id="note-html-container"
        v-if="currentHighlightedHtml"
        v-html="currentHighlightedHtml"
      ></div>
    </div>
    ...
    <div class="sub-container">
      ...
      >
        <div class="exp-btn">
          <svg
            @click="showFullTermContext(noteTIDPair[1])"
...
export default {
  ...
  methods: {
    ...
    highlightContexts(note) {
      ...
      if (isFirstHighlight) {
        doc.body.innerHTML =
          `<div class="no-context-alert">Could not parse the reference within this note.</div>` +
          doc.body.innerHTML
      }
    ...
    scrolledIndex: function (newVal, oldVal) {
      ...
      if (scrollHighlight) {
        ...
        if (scrollableParent) {
          ...
          let headerHeight = header ? header.clientHeight : 0
          scrollableParent.scrollTop =
            scrollHighlight.offsetTop - scrollableParent.offsetTop - headerHeight - 20
